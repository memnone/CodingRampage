...
#pragma once
#ifndef _RAMPAGE_SYS_XMLDATA_H_
#define _RAMPAGE_SYS_XMLDATA_H_
...
template <typename _Seq> void XMLGetValue(_Seq &seq, const std::string &name, const std::string &attr, rampage::XMLContext* context, bool create = false) {
	...
	for (typename XMLContext::XMLContextMap::const_iterator i = range.first; i != range.second; ++i) {
		...
		catch (const rampage::MsgXMLParserAttributeNotFound&) {}
	}
}
