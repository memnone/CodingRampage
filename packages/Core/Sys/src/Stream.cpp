...
#include <Rampage/Defs/Defs.h>
#include <Rampage/Defs/Types.h>
#include <Rampage/Sys/Stream.h>
#include <memory.h>
#include <algorithm>
...
template <> const Stream& rampage::operator >> (const Stream& stream, Serializable& serializable) {
	serializable.load(stream);
	return stream;
}

/** Output stream operator */
template <> Stream& rampage::operator << (Stream& stream, const Serializable& serializable) {
...
void rampage::mkdir(const char *_dir) {
	std::string dir(_dir);
	bool bName = false;
