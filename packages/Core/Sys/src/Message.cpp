...
char* rampage::vsnprintf(char* begin, const char* end, const char* format, va_list argptr) {
	if (begin >= end)
		return begin;
...
char* rampage::snprintf(char* begin, const char* end, const char* format, ...) {
	va_list argptr;
...
void Message::create(SecTmReal time, U32 thread, Level level, Code code, const char* format, va_list argptr) {
	...
	if (_time != TIME_UNDEF || _thread != THREAD_UNDEF || _level != LEVEL_UNDEF || _code != CODE_UNDEF) {
		...
		if (_time != TIME_UNDEF)
			begin = rampage::snprintf(begin, end, "time=%.6f, ", _time);
		
		if (_thread != THREAD_UNDEF)
			begin = rampage::snprintf(begin, end, "thread=%u", _thread);

		if (_level != LEVEL_UNDEF)
			begin = rampage::snprintf(begin, end, ", level=%s", LEVEL_STR[_level]);

		if (_code != CODE_UNDEF)
			begin = rampage::snprintf(begin, end, ", code=%u", _code);
	...
	_msg = begin;
	begin = rampage::vsnprintf(begin, end, format, argptr);
}
