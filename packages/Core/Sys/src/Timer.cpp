...
#ifdef __APPLE__
int rampage::clock_gettime(int clk_id, struct timespec* t) {
    struct timeval now;
...
void rampage::timevalnorm(timeval& tv) {
...
#ifndef WIN32
void rampage::timespecnorm(timespec& ts) {
...
timeval rampage::MSecToTimeval(MSecTmU32 t) {
...
#ifndef WIN32
MSecTmU32 rampage::TimespecToMSec(const timespec& ts) {
...
timespec rampage::MSecToTimespec(MSecTmU32 t) {
	if (t >= MSEC_TM_U32_MAX)
		return TIMESPEC_MAX;
...
int rampage::gettimeofday(struct timeval *tv, struct timezone *tz) {
	FILETIME ft;
...
timeval rampage::timevaldiff(const timeval& tv0, const timeval& tv1) {
	timeval tv = tv1;
